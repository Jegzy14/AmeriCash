...
FontResizeEvent = (function(window,document){
	...
	resizeFrame.title = 'font resize helper';
...
;(function($) {
	...
	$( "ul.nav-top li .drop" ).each(function( index ) {
		
		// add/remove the over class when clicked
		$(this).closest('li').find('a').first().click(function(e) {
		  e.preventDefault();
		  $li = $(this).closest('li');
		...
		//remove the hover class then the mouse goes out of focus
		...
		//remove the hover class when a different list item becomes in focus, if it's the same list item leave the hover class
		$(this).closest('li').focusin(function() {
		...
		//add the contact link as a sub menu item since the parent link will no longer take you there
		if($(this).closest('li').find('a').first().attr('href') == '/contact/'){
			$(this).closest('li').find('ul').first().prepend('<li><a href="/contact/" target="_self"><span>Contact Us</span></a></li>');
		}
	...
	$( "ul.nav-top > li" ).each(function() {
		$(this).focusin(function() {
			if($('body.search-active').length){
				$('.search-form #searchtext').focus();
			}
		});
	});
	
	//close the popup search if they tab off of the search button
	$('.search-form .submit').focusout(function() {
		$('.search-popup .close-btn').click();
		$('.search-opener').focus();
	});
	
	if($('.faq-flex-layout').length){
		
		//open the answer
		$( '.faq-flex-layout .opener' ).each(function( ) {
			$(this).focusin(function() {
				$(this).click();
			});
		});
		
		//get the screen reader to read the answer
		$( '.faq-flex-layout .beans-slide' ).each(function( ) {
			$(this).wrap( "<a href='#' style='color:#333;text-decoration:none;cursor:text'></a>" );
		});
	}
	
	//get the screen reader to read contact info
	if($('.contact-block').length){
		$('.contact-methods, .contact-info').wrap( "<a href='#' style='cursor:text'></a>" );
	};
